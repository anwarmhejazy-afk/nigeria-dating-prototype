...
type Subscription = { id: string; user_id: string; plan_slug: string; status: string; current_period_end: string | null; is_test: boolean };
type Transaction = { id: string; plan_slug: string; amount_minor: number; currency: string; status: string; created_at: string };
...
export function MonetizationDashboard({
  ...
  const revenueMinor = useMemo(() => transactions.filter((item) => item.status === "successful").reduce((sum, item) => sum + item.amount_minor, 0), [transactions]);
  ...
  const saveSettings = async () => {
    ...
    setMessage(response.ok ? "Monetisation settings saved." : payload.error || "Unable to save settings.");
    setBusy(false);
  };
  ...
  const grant = async () => {
    ...
    const payload = await response.json();
    setMessage(response.ok ? `${plan.toUpperCase()} test access granted. Refresh to see the updated totals.` : payload.error || "Unable to grant membership.");
    setBusy(false);
  };

  const inputClass = "mt-2 w-full rounded-2xl border border-white/10 bg-[#0d0f14] px-4 py-3 text-sm text-white outline-none focus:border-[#F2C94C]/60";
  ...
  return (
    <main className="min-h-screen bg-[#07080b] text-white">
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
        ...
        {!flutterwaveConfigured && <div className="mt-5 rounded-3xl border border-blue-400/20 bg-blue-400/[0.06] p-5 text-sm leading-6 text-blue-100/70">Flutterwave API keys are not added yet. Checkout remains disabled and no money can be collected. Use the test-membership tool below to test Premium and VIP features safely.</div>}
        ...
        <div className="mt-6 grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
          <section className="rounded-[30px] border border-white/[0.08] bg-white/[0.025] p-6">
            ...
            <h1 className="mt-2 text-2xl font-black">Pricing and split settlement</h1>
            <p className="mt-2 text-sm leading-6 text-white/38">The 1:1 ratio represents a 50/50 split of the settlement amount after Flutterwave fees. Keep checkout disabled until both subaccounts, test plans and webhook secret are ready.</p>
          ...
          <section className="rounded-[30px] border border-white/[0.08] bg-white/[0.025] p-6">
            <p className="text-[10px] font-black tracking-[0.25em] text-[#F2C94C]">SAFE FEATURE TESTING</p>
            <h2 className="mt-2 text-2xl font-black">Grant test membership</h2>
            <p className="mt-2 text-sm leading-6 text-white/38">This does not charge money. It lets a normal member test all Premium or VIP controls.</p>
            <div className="mt-6 space-y-4">
              ...
              <Field label="Plan"><select className={inputClass} value={plan} onChange={(event) => setPlan(event.target.value)}><option value="premium">Premium</option><option value="vip">VIP</option><option value="free">Return to Free</option></select></Field>
              <Field label="Number of days"><input type="number" min="1" max="365" className={inputClass} value={days} onChange={(event) => setDays(Number(event.target.value))} /></Field>
            </div>
...
function Metric({ label, value, hint }: { label: string; value: string; hint: string }) { return <div className="rounded-3xl border border-white/[0.08] bg-white/[0.025] p-5"><p className="text-3xl font-black text-[#FFE58C]">{value}</p><p className="mt-2 text-sm font-black">{label}</p><p className="mt-1 text-xs text-white/35">{hint}</p></div>; }
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="block"><span className="text-[10px] font-black uppercase tracking-[0.16em] text-white/35">{label}</span>{children}</label>; }
