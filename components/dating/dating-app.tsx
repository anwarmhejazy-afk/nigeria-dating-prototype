...
import { calculateAge, initialsFromName, type MemberProfile } from "@/lib/profile";
import type { PointerEvent as ReactPointerEvent, SVGProps } from "react";
import { useMemo, useState } from "react";
import { notifications, profiles, type Profile } from "@/app/data";
...
export function DatingApp({ memberProfile }: { memberProfile: MemberProfile }) {
    ...
    <main className="flex min-h-screen items-center justify-center p-0 sm:p-4">
      <section className="relative flex h-[100dvh] w-full max-w-[430px] flex-col overflow-hidden bg-[#0d0f14] shadow-[0_0_100px_rgba(242,201,76,0.18)] sm:h-[min(860px,calc(100vh-32px))] sm:rounded-[38px] sm:border sm:border-white/10">
        ...
        <div className="relative z-10 flex min-h-0 flex-1 flex-col">
          {tab === "home" && <HomeScreen memberProfile={memberProfile} search={search} setSearch={setSearch} filteredProfiles={filteredProfiles} openDetails={openDetails} setTab={setTab} setOverlay={setOverlay} />}
          ...
          {tab === "profile" && <ProfileScreen memberProfile={memberProfile} editProfile={() => router.push("/profile/edit")} setOverlay={setOverlay} showToast={showToast} />}
        </div>
...
function HomeScreen({ memberProfile, search, setSearch, filteredProfiles, openDetails, setTab, setOverlay }: { memberProfile: MemberProfile; search: string; setSearch: (value: string) => void; filteredProfiles: Profile[]; openDetails: (profile: Profile) => void; setTab: (tab: MainTab) => void; setOverlay: (overlay: Overlay) => void; }) {
  return (
    <div className="app-scroll min-h-0 flex-1 overflow-y-auto px-5 pb-6 pt-5">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-white/45">Good evening — {memberProfile.city || "Nigeria"} is matching.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setTab("profile")} className="relative flex h-11 w-11 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-[#F2C94C] to-[#b8860b] text-sm font-black text-black shadow-[0_0_22px_rgba(242,201,76,0.2)]">{memberProfile.avatar_url ? <Image src={memberProfile.avatar_url} alt={memberProfile.display_name} fill sizes="44px" className="object-cover" /> : initialsFromName(memberProfile.display_name)}</button>
        </div>
      </header>
...
function ProfileScreen({ memberProfile, editProfile, setOverlay, showToast }: { memberProfile: MemberProfile; editProfile: () => void; setOverlay: (overlay: Overlay) => void; showToast: (text: string) => void; }) {
  const age = calculateAge(memberProfile.date_of_birth);
  const location = [memberProfile.city, memberProfile.state].filter(Boolean).join(", ") || "Nigeria";
  const menu = [
    { title: "Edit profile", subtitle: "Photos, bio and personal details", icon: "user" as IconName, action: editProfile },
    { title: "Get verified", subtitle: "Build trust with a verified badge", icon: "shield" as IconName, action: () => showToast("Verification flow is coming next") },
  ...
  return (
    <div className="app-scroll min-h-0 flex-1 overflow-y-auto px-5 pb-6 pt-5">
      ...
      <section className="mt-5 overflow-hidden rounded-[30px] border border-white/10 bg-gradient-to-br from-[#1d1a11] via-[#15161b] to-[#111318] p-5 shadow-2xl">
        <div className="flex items-center gap-4">
          <div className="relative">
            <div className="relative flex h-24 w-24 items-center justify-center overflow-hidden rounded-[28px] bg-gradient-to-br from-[#F2C94C] via-[#E3AC25] to-[#8B6508] text-2xl font-black text-black">
              {memberProfile.avatar_url ? <Image src={memberProfile.avatar_url} alt={memberProfile.display_name} fill sizes="96px" className="object-cover" /> : initialsFromName(memberProfile.display_name)}
            </div>
            {memberProfile.is_verified && <span className="absolute -bottom-1 -right-1 flex h-8 w-8 items-center justify-center rounded-full border-4 border-[#17171a] bg-[#4DA3FF]"><Icon name="check" className="h-3.5 w-3.5" /></span>}
          </div>
          <div className="min-w-0 flex-1"><h2 className="truncate text-2xl font-black">{memberProfile.display_name}{age !== null ? `, ${age}` : ""}</h2><p className="mt-1 truncate text-xs font-semibold text-[#F2C94C]">{memberProfile.occupation || "NAIJA MATCH Member"} · {location}</p><p className="mt-2 text-xs text-white/45">Profile visible · Active now</p></div>
        </div>
        <div className="mt-5"><div className="flex items-center justify-between text-xs"><span className="font-bold">Profile strength</span><span className="font-black text-[#F2C94C]">{memberProfile.profile_completion}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-[#F2C94C] to-[#FFE58C]" style={{ width: `${memberProfile.profile_completion}%` }} /></div><p className="mt-2 text-[10px] text-white/35">{memberProfile.photo_urls.length < 4 ? "Add more photos to increase trust and visibility." : "Your profile is ready to make a strong impression."}</p></div>
      </section>
